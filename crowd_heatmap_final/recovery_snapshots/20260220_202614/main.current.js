...
function isDashboardPage() {
    ...
    return path.includes('/dashboard/') || path.includes('/business-recommendations/');
}
...
function renderPopularPlacesFlashcards(places) {
    ...
    if (!places || places.length === 0) {
        container.innerHTML = `<div class="text-center text-muted p-4">No popular places found nearby.</div>`;
        return;
    }

    const limitedPlaces = places.slice(0, 16); // Max 16 for true 4x4 grid (4 rows)
            ...
            <div class="flashcard-body">
                ...
                <div class="flashcard-details-grid mt-3">
                    <div class="detail-item">
                        <span class="detail-label">Peak Revenue</span>
    ...
    if (row) {
        row.innerHTML = `
            <div class="text-center text-muted p-5">
                ...
                <p>${message}</p>
                <a href="/dashboard/" class="btn btn-primary-blue mt-3">Go to Map</a>
            </div>
        `;
    }
}
...
async function initDashboardAIFromStorage() {
    ...
    if (!stored) {
        setDashboardAIEmptyState('Select a location from map first');
        if (forceAI) {
            ...
            if (dynamicMapContainer) dynamicMapContainer.classList.remove('d-none');
    ...
    if (forceAI && document.getElementById('dashboard-map')) {
        await selectSearchResult(stored.lat, stored.lng, stored.name || 'Selected Location', stored.type || '');
        await findPopularPlaces(stored.lat, stored.lng, false);
    } else {
...
function createBusinessCard(place, type, index, fallbackLat, fallbackLon) {
    ...
    if (isPopular) {
        buttonHtml = `
            <button type="button" class="btn-track-full" onclick="selectSearchResult(${finalLat}, ${finalLon}, '${escapedName}', '${placeBusinessType}')">
                Track Location
            </button>`;
    } else {
        buttonHtml = `
